...
function App() {
  ...
  const result = useMemo(() => selectedPair ? solveSteam(steamInputs) : { error: '체크박스에서 서로 계산 가능한 상태량 2개를 선택하세요. 예: P+T, P+h, P+s, P+x, T+h, T+s', warnings: [] }, [steamInputs, selectedPair]);
  ...
  const velocity = state ? calculateVelocity(pipeFlow, pipeFlowUnit, state.specificVolume, resolvedPipeRow.idMm) : undefined;
  ...
  return <main className={mini ? 'app mini' : 'app'}>
    <header className="hero">
      <div>
        ...
        <h1>SteamWise</h1>
        <p className="subtitle">공학책 스팀테이블처럼 상태량 2개를 체크하고 입력하면, 체크하지 않은 나머지 P·T·h·s·x·v 값을 자동 계산합니다.</p>
      </div>
    ...
    {!mini && <div className="toolsGrid">
      <section className="panel" id="pipe-sizing">
        <h2>Pipe code / size / schedule</h2>
        <p className="muted">코드 목록에서 먼저 표준을 고른 뒤, 해당 코드의 사이즈와 스케줄을 순서대로 선택합니다.</p>
        <div className="pipeCascade">
          ...
          <label>Nominal size<select value={normalizedPipeNps} onChange={(e) => { const nps = e.target.value; const firstSchedule = getPipeSchedules(pipeStandard, nps)[0]; setPipeNps(nps); setPipeSchedule(firstSchedule); }}>{pipeSizes.map((nps) => <option key={nps} value={nps}>{nps} / DN{PIPE_SIZES.find((row) => row.standard === pipeStandard && row.nps === nps)?.dn}</option>)}</select></label>
        ...
        <div className="pipeSummary">OD {resolvedPipeRow.odMm} mm · ID {resolvedPipeRow.idMm} mm {resolvedPipeRow.note ? `· ${resolvedPipeRow.note}` : ''}</div>
        ...
        {velocity && <div className="resultCards compact"><Metric label="Velocity" value={format(velocity.velocityMS)} unit="m/s" /><Metric label="Vol. flow" value={format(velocity.volumetricM3S)} unit="m³/s" /><Metric label="Pipe ID" value={format(resolvedPipeRow.idMm)} unit="mm" /></div>}
      </section>
...
function SteamTableRow({ field, checked, disabled, value, inputs, onToggle, onSet }: { field: SteamTableField; checked: boolean; disabled: boolean; value: string; inputs: SteamInputs; onToggle: () => void; onSet: <K extends keyof SteamInputs>(key: K, value: SteamInputs[K]) => void }) {
  ...
  return <div className={checked ? 'steamRow checked' : 'steamRow'}>
    ...
    <div className="steamValue">{checked ? <SteamFieldInput field={field} inputs={inputs} onSet={onSet} /> : <output>{value}</output>}</div>
  </div>;
}
...
function SteamFieldInput({ field, inputs, onSet }: { field: SteamTableField; inputs: SteamInputs; onSet: <K extends keyof SteamInputs>(key: K, value: SteamInputs[K]) => void }) {
  ...
  if (field === 'entropy') return <NumberInput label="" value={inputs.entropy} onValue={(v) => onSet('entropy', v)} />;
  if (field === 'quality') return <NumberInput label="" value={inputs.quality} onValue={(v) => onSet('quality', v)} />;
